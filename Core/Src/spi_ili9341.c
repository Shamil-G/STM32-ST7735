...
#define END_OPERATION while(__HAL_SPI_GET_FLAG(&HSPI,SPI_FLAG_BSY)!=RESET)
#define READY_DATA_REGISTR __HAL_SPI_GET_FLAG(&HSPI, SPI_FLAG_TXE)
...
uint8_t ST7735_WriteData(uint8_t* pData, uint32_t Size, uint32_t Timeout)
{
	  uint32_t tickstart;
...
void ili9341_SendCommand(uint8_t cmd){
	...
	ST7735_WriteData(&cmd, 1, 5000);
	//*((__IO uint8_t *)&HSPI.Instance->DR) = cmd;
...
static void ili9341_SendData(uint8_t* buff, uint16_t buff_size){
	...
	ST7735_WriteData(buff, buff_size, 1000000);
}
//-------------------------------------------------------------------
static void ili9341_SendDataByte(uint8_t buff){
	...
	//*((__IO uint8_t *)&HSPI.Instance->DR) = buff;
	//HAL_SPI_Transmit(&hspi2, &buff, 1, HAL_MAX_DELAY);
	ST7735_WriteData(&buff, 1, 1000000);
}
//--------------------------------------------------------------------
